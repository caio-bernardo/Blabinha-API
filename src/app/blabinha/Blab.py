...
class Variaveis:
    def __init__(self, turn: int, input: str, bonus: int, stars: int, heroFeatures: list[str], repetition: int):
        self.turn = turn
        self.input = input
        self.answer = ""
        self.bonus = bonus
...
class Blab:
    ...
    def __init__(self, api_key: str, modelo: str, chatRepository: ChatRepository, chat_id: int):
        self.modelo = modelo
    ...
    def escolheParte(self, variaveis: Variaveis):
        ...
        if 100 <= secao < 200:
            ...
            variaveis.answer = "Este chat está encerrado pois ocorreu um erro. Erro de seção tipo 04"
            resposta = variaveis
        return resposta
        ...
        if variaveis.turn in quests:
            ...
            response2 = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "user", "content": variaveis.input },
                    {"role": "assistant", "content": response.choices[0].message.content},
                    {"role": "system", "content": "Dado o assunto tratado gere uma questão contendo 4 alternativas e uma só resposta correta enumere elas de 1) a 4). Fale somente a questão e as alternativas, não passe a resposta."},
                ]
            )
    ...
    def secao310(self,variaveis: Variaveis):
        ...
        if not bonus:
            ...
            messages=[
                {"role": "system", "content": "Você é um robô chamado Blabinha e está conversando com uma criança." },
                {"role": "system", "content": "Diga que a pessoa ganhou achou um bônus e por isso ganhou duas estrelas! Termine dizendo que a ferramenta :" + bonus + "vai ser usada pelo super-herói."},
    ...
    def getHeroFeature(self, variaveis: Variaveis) -> str:
        ...
        if tamanho >= 3:
            frase_final =  frase_final + ". O herói tem uma capa com as seguintes características:" + lista[2]
        if tamanho >=2:
            frase_final = frase_final + ". A roupa do herói tem as seguintes características :" + lista[1]
    ...
    def secao350(self,variaveis: Variaveis):
        ...
        messages=[
            {"role": "system", "content": "Você é um robô chamado Blabinha e está conversando com uma criança." },
            {"role": "system", "content": " Diga que a pessoa conseguiu " + str(variaveis.stars)+" estrelas de um total de 10." +
        ...
        frase =  "Gere um heroi que vai proteger o oceano do Brasil. Ele tem as seguintes caracteristicas:" + prompt + "A sua força está ligada com a quantidade de pontos. Ele tem" + str(variaveis.stars) + "pontos de um total de 10."
